package es.rfgr.cr4vegas.modelo.postgresql.bd.grupo;
...
public class GrupoBD {
	...
	private final String tableName = "public.grupo";
	...
	public GrupoPK insert(Grupo dto) throws SQLException {
		try {
	...
	public void update(GrupoPK pk, Grupo dto) throws SQLException {
		try {
			initializeTimeConnSuppliedConn();
	...
	public void delete(GrupoPK pk) throws SQLException {
		try {
			initializeTimeConnSuppliedConn();
	...
	public ObservableList<Grupo> findAll() throws SQLException {
		return findByDynamicSelect(SQL_SELECT + " ORDER BY nombre", null);
	}
	
	public Grupo findByPrimaryKey(GrupoPK pk) throws SQLException {
		return findByPrimaryKey( pk.getNombre() );
	}

	public Grupo findByPrimaryKey(String nombre) throws SQLException {
		ObservableList<Grupo> grupos = findByDynamicSelect( SQL_SELECT + " WHERE nombre = ?", new Object[] { nombre } );
	...
	private void populateDto(Grupo dto, ResultSet rs) throws SQLException {
		dto.setNombre(rs.getString(COLUMN_NOMBRE));
	}

	private ObservableList<Grupo> fetchMultiResults(ResultSet rs) throws SQLException {
		ObservableList<Grupo> resultList = FXCollections.observableArrayList();
		while (rs.next()) {
			Grupo dto = new Grupo();
			populateDto(dto, rs);
			resultList.add(dto);
	...
	private ObservableList<Grupo> findByDynamicSelect(String sql, Object[] sqlParams) throws SQLException {
		initializeTimeConnSuppliedConn();
