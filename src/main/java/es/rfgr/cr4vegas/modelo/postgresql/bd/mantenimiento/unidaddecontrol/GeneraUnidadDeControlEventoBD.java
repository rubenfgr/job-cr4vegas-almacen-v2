...
public class GeneraUnidadDeControlEventoBD {

	private Connection userConn;

	private final String tableName = "public.generaudcontrolevento";
	...
	public GeneraUnidadDeControlEventoBD() {
	...
	public GeneraUnidadDeControlEventoPK insert(GeneraUnidadDeControlEvento dto) throws SQLException {
		try {
	...
	public void update(GeneraUnidadDeControlEventoPK pk, GeneraUnidadDeControlEvento dto) throws SQLException {
		try {
			initializeTimeConnSuppliedConn();
	...
	public void delete(GeneraUnidadDeControlEventoPK pk) throws SQLException {
		try {
			initializeTimeConnSuppliedConn();
	...
	public ObservableList<GeneraUnidadDeControlEvento> findAll() throws SQLException {
		return findByDynamicSelect(SQL_SELECT + " ORDER BY codevento", null);
	}

	public GeneraUnidadDeControlEvento findByPrimaryKey(GeneraUnidadDeControlEventoPK pk) throws SQLException {
		return findByPrimaryKey(pk.getCodEvento());
	}

	public GeneraUnidadDeControlEvento findByPrimaryKey(int codEvento) throws SQLException {
		ObservableList<GeneraUnidadDeControlEvento> uDControlEventos = findByDynamicSelect(
				SQL_SELECT + " WHERE codevento = ?", new Object[] { Integer.valueOf(codEvento) });
		return uDControlEventos.size() == 0 ? null : uDControlEventos.get(0);
	}
	...
	private void populateDto(GeneraUnidadDeControlEvento dto, ResultSet rs) throws SQLException {
		dto.setCodEvento(rs.getInt(COLUMN_CODEVENTO));
		dto.setCodMan(rs.getInt(COLUMN_CODMAN));
	}

	private ObservableList<GeneraUnidadDeControlEvento> fetchMultiResults(ResultSet rs) throws SQLException {
		ObservableList<GeneraUnidadDeControlEvento> resultList = FXCollections.observableArrayList();
		while (rs.next()) {
			GeneraUnidadDeControlEvento dto = new GeneraUnidadDeControlEvento();
			populateDto(dto, rs);
			resultList.add(dto);
		}
		return resultList;
	...
	private ObservableList<GeneraUnidadDeControlEvento> findByDynamicSelect(String sql, Object[] sqlParams)
			throws SQLException {
		initializeTimeConnSuppliedConn();
