...
public class FamiliaBD {
	...
	private final String tableName = "public.familia";
	...
	public FamiliaPK insert(Familia dto) throws SQLException {
		try {
	...
	public void update(FamiliaPK pk, Familia dto) throws SQLException {
		try {
			initializeTimeConnSuppliedConn();
	...
	public void delete(FamiliaPK pk) throws SQLException {
		try {
			initializeTimeConnSuppliedConn();
	...
	public ObservableList<Familia> findAll() throws SQLException {
		return findByDynamicSelect(SQL_SELECT + " ORDER BY nombre", null);
	}
	
	public Familia findByPrimaryKey(FamiliaPK pk) throws SQLException {
		return findByPrimaryKey(pk.getNombre());
	}

	public Familia findByPrimaryKey(String nombre) throws SQLException {
		ObservableList<Familia> familias = findByDynamicSelect( SQL_SELECT + " WHERE nombre = ?", new Object[] {  nombre } );
		return familias.size() == 0 ? null : familias.get(0);
	}
	...
	private void populateDto(Familia dto, ResultSet rs) throws SQLException {
		dto.setNombre(rs.getString(COLUMN_NOMBRE));
	}

	private ObservableList<Familia> fetchMultiResults(ResultSet rs) throws SQLException {
		ObservableList<Familia> resultList = FXCollections.observableArrayList();
		while (rs.next()) {
	...
	private ObservableList<Familia> findByDynamicSelect(String sql, Object[] sqlParams) throws SQLException {
		initializeTimeConnSuppliedConn();
