...
public class UbicacionBD {
	...
	private final String tableName = "public.ubicacion";
	...
	public UbicacionPK insert(Ubicacion dto) throws SQLException {
		try {
	...
	public void update(UbicacionPK pk, Ubicacion dto) throws SQLException {
		try {
			initializeTimeConnSuppliedConn();
	...
	public void delete(UbicacionPK pk) throws SQLException {
		try {
			initializeTimeConnSuppliedConn();
	...
	public ObservableList<Ubicacion> findAll() throws SQLException {
		return findByDynamicSelect(SQL_SELECT + " ORDER BY nombre", null);
	}

	public Ubicacion findByPrimaryKey(UbicacionPK pk) throws SQLException {
		return findByPrimaryKey(pk.getNombre());
	}

	public Ubicacion findByPrimaryKey(String nombre) throws SQLException {
		ObservableList<Ubicacion> ubicaciones = findByDynamicSelect(SQL_SELECT + " WHERE nombre = ?",
				new Object[] { nombre });
		return ubicaciones.size() == 0 ? null : ubicaciones.get(0);
	}
	...
	private void populateDto(Ubicacion dto, ResultSet rs) throws SQLException {
		dto.setNombre(rs.getString(COLUMN_NOMBRE));
	}

	private ObservableList<Ubicacion> fetchMultiResults(ResultSet rs) throws SQLException {
		ObservableList<Ubicacion> resultList = FXCollections.observableArrayList();
		while (rs.next()) {
	...
	private ObservableList<Ubicacion> findByDynamicSelect(String sql, Object[] sqlParams) throws SQLException {
		initializeTimeConnSuppliedConn();
